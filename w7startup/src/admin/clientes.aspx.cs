...
//        {
//            DbCommand command = db.GetSqlStringCommand(
//   "UPDATE cliente SET razao_social = @razao_social, inscricao_estadual = @inscricao_estadual, celular = @celular, nomecompleto = @nomecompleto, cep = @cep, endereco = @endereco, bairro = @bairro, numero = @numero, cidade = @cidade, estado = @estado, complemento = @complemento, status = @status where id = @id");
