    ...
    public partial class viewpedido : System.Web.UI.Page
    {
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            using (IDataReader reader = DatabaseFactory.CreateDatabase("ConnectionString").ExecuteReader(CommandType.Text,
                              "select p.id, p.observacao, p.valor, p.rastreio, p.prazo_entrega, p.data_entrega, FORMAT(p.DATACADASTRO,'dd/MM/yyyy') as datacadastro, p.[status], c1.nomecompleto as nomefantasia, 'CPF/CNPJ: '+ c2.cnpj_cpf as cpf, 'CEP: '+ c1.cep+' - '+ c1.cidade+'/'+c1.estado as local_lojista, c1.celular as telefonelojista, c2.nomecompleto as nomecliente, c1.cnpj_cpf as CPFLojista, 'CEP: '+ c2.CEP+' - '+ c2.endereco+' '+c2.bairro+' '+c2.numero+','+c2.cidade+'/'+c2.estado as enderecocliente from pedido p join cliente c1 on c1.id = p.idlojista join cliente c2 on c2.id = p.idconsumidor where p.id = '" + hdfId.Value + "'"))
            {
                if (reader.Read())
                {
                    ...
                    lblObservações.Text = reader["observacao"].ToString();
                }
            }
        }
