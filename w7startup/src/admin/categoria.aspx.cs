    ...
    public partial class categoria : System.Web.UI.Page
    {
        ...
        protected void gdvDados_RowCommand(object sender, System.Web.UI.WebControls.GridViewCommandEventArgs e)
        {
            ...
            using (IDataReader reader = DatabaseFactory.CreateDatabase("ConnectionString").ExecuteReader(CommandType.Text,
                          "SELECT * from categoria where id = '" + hdfId.Value + "'"))
            {
                if (reader.Read())
                {
        ...
        protected void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            {
                DbCommand command = db.GetSqlStringCommand(
               "UPDATE categoria SET descricao = @descricao, status = @status where id = @id");
        ...
        {
            sdsDados.SelectCommand = "select * from categoria where descricao like '%" + txtBuscar.Text+"%'";
            gdvDados.DataBind();
        }
    }
}