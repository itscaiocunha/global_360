<%@ Page Async="true" Title="" Language="C#" MasterPageFile="~/src/admin/principal.Master" AutoEventWireup="true" CodeBehind="pedidos.aspx.cs" Inherits="global.pedidos" %>
...
<i data-acorn-icon="search"></i> Filtrar</asp:LinkButton>     
    ...
    <ItemTemplate>
        <asp:HyperLink ID="HyperLink2" runat="server" Target="_blank" NavigateUrl='<%# "printpedido.aspx?id="+ Eval("id") %>'>Imprimir</asp:HyperLink>
                    ...
                    <asp:SqlDataSource ID="sdsDados" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="select p.id, max(c2.nomecompleto) as vendedor, max(c.nomecompleto) as comprador, sum(pp.valor) as valor, sum(pp.qtde) as qtde, FORMAT(max(p.DATACADASTRO),'dd/MM/yyyy') as datacadastro, max(p.status) as status from pedido p
join pedido_produto pp on pp.idpedido = p.id
join cliente c on c.id = p.idconsumidor
join cliente c2 on c2.id = p.idlojista
where c.idtipocliente in (1,2)
